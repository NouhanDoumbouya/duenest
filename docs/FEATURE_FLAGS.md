...
merge), `document_page_extract` (export PDF pages), and `document_redaction`
(flatten + redact an existing PDF, experimental). These stay invisible to normal
users until a founder launches each one (`beta_only` / `enabled`). They mostly gate UI
