...
FEATURE_DEFINITIONS: list[dict] = [
    ...
    {"key": "quick_share_code", "name": "Quick Share — CertaNest code", "default": Visibility.ENABLED, "description": "Receive-by-code claiming."},
    ...
    {"key": "verified_shares", "name": "Verified shares", "default": Visibility.FOUNDER_ONLY, "description": "Tamper-evident, CertaNest-signed shares with a public /verify page. Provenance + integrity only."},
    {"key": "share_requests", "name": "Share Requests", "default": Visibility.BETA_ONLY, "description": "Request documents from someone; they fulfil the checklist from their vault. Delivered via the Quick Share engine."},
    ...
    {"key": "subscriptions", "name": "Subscriptions (deprecated)", "default": Visibility.DISABLED, "description": "DEPRECATED — legacy Subscription Radar (finance/recurring-payment tracking). CertaNest is a life-document readiness platform, not a subscription/finance tracker. Disabled by default; data tables are retained but no longer exposed. Recurring renewals now live in Deadlines & Renewals."},
    ...
    {"key": "scanner_advanced_tools", "name": "Scanner — advanced tools", "default": Visibility.BETA_ONLY, "description": "Master gate for advanced scanner/document-preparation tools."},
    {"key": "scan_to_safesend", "name": "Scanner — share safely", "default": Visibility.BETA_ONLY, "description": "Continue a scanned document into Quick Share / SafeSend."},
    {"key": "scan_to_bundle", "name": "Scanner — add to bundle", "default": Visibility.BETA_ONLY, "description": "Add a scanned document to an application/renewal bundle."},
    {"key": "scan_to_reminder", "name": "Scanner — add reminder", "default": Visibility.BETA_ONLY, "description": "Create an expiry/deadline reminder from a scanned document."},
    {"key": "scan_safe_copy", "name": "Scanner — prepare copy", "default": Visibility.BETA_ONLY, "description": "Create a prepared copy (the original is never modified)."},
    {"key": "scan_watermark", "name": "Scanner — watermark", "default": Visibility.BETA_ONLY, "description": "Burn a label watermark into a prepared/exported copy."},
    {"key": "scan_compression", "name": "Scanner — compress PDF", "default": Visibility.BETA_ONLY, "description": "Create a smaller PDF copy for uploads/portals."},
    {"key": "scan_page_export", "name": "Scanner — export selected pages", "default": Visibility.BETA_ONLY, "description": "Export only selected scanned pages as a new copy (also covers combining scanned pages into one PDF)."},
    ...
    {"key": "document_merge", "name": "Documents — merge PDFs", "default": Visibility.BETA_ONLY, "description": "Combine selected File Inbox PDFs into one new PDF (client-side; originals preserved)."},
    {"key": "document_page_extract", "name": "Documents — export PDF pages", "default": Visibility.BETA_ONLY, "description": "Export selected pages of a File Inbox PDF as a new PDF (client-side; original preserved)."},
    ...
    {"key": "scan_pdf_import", "name": "Scanner — import PDF", "default": Visibility.BETA_ONLY, "description": "Import an existing PDF into the scanner (rasterized to pages) for re-export/preparation."},
    {"key": "document_compress", "name": "Documents — shrink PDF", "default": Visibility.BETA_ONLY, "description": "Make a smaller image-based copy of a scanned/image-heavy File Inbox PDF (client-side; original preserved)."},
    # --- Scanner Document Organization v2 -------------------------------
    {"key": "duplicate_detection", "name": "Documents — duplicate detection", "default": Visibility.BETA_ONLY, "description": "Warn before adding a file that matches an existing one (checksum/name/size). Never auto-deletes or replaces."},
    {"key": "document_versioning", "name": "Documents — version history", "default": Visibility.BETA_ONLY, "description": "View a document's version history and restore previous metadata. Originals/previous files are preserved."},
    {"key": "filename_templates", "name": "Scanner — filename templates", "default": Visibility.BETA_ONLY, "description": "Quick clean-name chips (document type + year) when saving a scan. Editable; nothing invented."},
    {"key": "scan_modes", "name": "Scanner — scan modes", "default": Visibility.BETA_ONLY, "description": "Friendly presets (Document/ID/Certificate/Receipt/Application/Photo) that set safe filter + quality defaults. User-chosen; not auto-recognition."},
    {"key": "scan_ocr", "name": "Scanner — extract text (OCR)", "default": Visibility.BETA_ONLY, "description": "On-device text extraction (Tesseract.js) from a scanned page. Runs in the browser; no image leaves the device. User-triggered; nothing is auto-filled."},
    {"key": "scan_hands_free", "name": "Scanner — hands-free batch", "default": Visibility.BETA_ONLY, "description": "Continuous auto-capture: once a page is framed/steady it is captured and committed, then the camera keeps going for the next page. Opt-in toggle; manual capture is unchanged."},
    {"key": "advanced_document_preview", "name": "Documents — advanced preview", "default": Visibility.BETA_ONLY, "description": "Image preview zoom controls (fit / actual size / +-) in the file viewer. UI-only."},
    {"key": "batch_scan_actions", "name": "File Inbox — batch actions", "default": Visibility.BETA_ONLY, "description": "Move multiple selected inbox files to the Vault at once (optionally under a category). Uses existing create-document; originals become documents."},
    ...
    {"key": "application_pack_preparation", "name": "Application packs — preparation", "default": Visibility.BETA_ONLY, "description": "Master gate for application-pack preparation features built on bundles."},
    {"key": "application_pack_templates", "name": "Application packs — templates", "default": Visibility.BETA_ONLY, "description": "Start a bundle from a generic, editable checklist template (scholarship/visa/university/job/travel/renewal). Suggestions only — never presented as official; requirements always vary."},
    {"key": "application_pack_timeline", "name": "Application packs — activity timeline", "default": Visibility.BETA_ONLY, "description": "Owner-only activity feed for a bundle (created, template applied, document attached/replaced/removed, status changed, exported). No file contents; never public."},
    {"key": "application_pack_safesend", "name": "Application packs — share via SafeSend", "default": Visibility.BETA_ONLY, "description": "Shortcut from a pack into the SafeSend (Quick Share) draft with the pack preselected. No link is created until the user confirms inside SafeSend."},
    ...
    {"key": "vault_bulk_actions", "name": "Vault — bulk actions", "default": Visibility.BETA_ONLY, "description": "Select multiple documents in the Vault to move to a category, archive, or move to Trash at once. Reuses existing per-document endpoints; destructive actions confirm first."},
    {"key": "vault_trash_undo", "name": "Vault — undo toast", "default": Visibility.BETA_ONLY, "description": "Show an inline 'Undo' toast after moving a document to Trash or archiving it (single or bulk), reversing via the existing restore / lifecycle endpoints."},
    {"key": "vault_smart_views", "name": "Vault — Smart Views", "default": Visibility.BETA_ONLY, "description": "A Smart Views panel on the Vault overview (Expiring soon, Needs review, Shared, In packs, Pinned, Archived, …) with real counts, linking into the pre-filtered documents list. State-based filtering only — no AI."},
    {"key": "vault_table_view", "name": "Vault — table view", "default": Visibility.BETA_ONLY, "description": "A compact table view mode for the documents list (name/category/type/expiry/status) alongside the existing list and grid views. Horizontally scrollable on mobile."},
    ...
    {"key": "qr_customization", "name": "QR — custom colors + scan warnings", "default": Visibility.BETA_ONLY, "description": "Custom foreground/background QR colors with live contrast + scan-reliability warnings on the SafeSend link screen. UI-only; the QR still encodes only the secure SafeSend URL and follows the same access rules."},
    ...
    {"key": "application_tracker", "name": "Application Tracker", "default": Visibility.FOUNDER_ONLY, "description": "Track the lifecycle of applications/renewals (scholarship, visa, job, grant, permit, …): status, deadline, linked application pack readiness, and deterministic next actions. No AI. Available to Free and Pro (Free: 3 active applications; Pro: 100). Founder-only until launched."},
    {"key": "smart_profile", "name": "Smart Profile", "default": Visibility.FOUNDER_ONLY, "description": "Per-user reusable application data (education, work, skills, achievements, common answers + extras) with a deterministic completeness score. Sensitive identity/document numbers stay in the encrypted profile-details store. No AI; owner-only; available to Free and Pro. Founder-only until launched."},
    {"key": "application_document_generation", "name": "AI — Application document generator", "default": Visibility.FOUNDER_ONLY, "description": "Generate professional application documents (ATS resume, CVs, cover/motivation letters, statement of purpose, application/recommendation emails, explanation letters) from Smart Profile + application + pack context, then review, pick a template, and export real PDF/DOCX saved to the pack/vault. Generate -> review -> export -> save; nothing auto-saves. Pro-only; AI credits per generation (email 3 / letter 5 / SOP & CV 8), charged only on success. Founder-only until launched."},
    {"key": "magic_inbox", "name": "Magic Inbox", "default": Visibility.FOUNDER_ONLY, "description": "One smart place to drop files, paste emails/requirement text, or paste a link. CertaNest captures the item, then proposes review-before-apply suggestions (save to vault, create pack/application, add requirements, set a deadline, import a requirement link). Manual intake + apply work without AI and are available to Free and Pro. No Gmail/Outlook integration in V1. Founder-only until launched."},
    {"key": "magic_inbox_triage", "name": "AI — Magic Inbox triage", "default": Visibility.FOUNDER_ONLY, "description": "Optional Claude smart triage of a Magic Inbox item: classify it, detect a deadline, and suggest pack/application/requirement/reminder actions with source snippets. Capture -> analyze -> review -> apply: nothing is created until the user approves; applying suggestions never calls AI. Pro-only; 3 AI credits per successful analysis, charged only on success. Founder-only until launched."},
    {"key": "redaction_watermarking", "name": "Redaction + watermarking", "default": Visibility.FOUNDER_ONLY, "description": "Create a protected COPY of a PDF/image: burn manual redaction rectangles into the pixels (PDFs are flattened so underlying text is destroyed — not extractable) and/or apply a watermark. The original file is never modified; the protected copy is a new encrypted private file that can be shared in a Sharing Room instead of the original. Deterministic (no AI); no automatic PII detection in V1. Founder-only until launched."},
    {"key": "b2b_portals", "name": "B2B Portals", "default": Visibility.FOUNDER_ONLY, "description": "Organization-facing portal workspace: manage people (clients/students/applicants/employees), open document cases, and orchestrate the existing primitives (packs/checklists, Sharing Rooms, Document Request Links) to collect and review documents per case. Deterministic (no AI); reuses existing upload/room/request systems (no duplicates). Founder/beta-gated MVP."},
    # --- Integrations OAuth Foundation V1 -------------------------------
    # Master gate for the integrations settings page + OAuth foundation, plus a
    # per-provider gate and per-import-flow gates. ALL default FOUNDER_ONLY so the
    # surface is invisible to normal users until deliberately launched. This branch
    # is the OAuth/account foundation only — NO Drive/Calendar/Gmail import, no
    # background sync, no write-back. The import keys gate FUTURE flows (not built).
    {"key": "integrations", "name": "Integrations", "default": Visibility.FOUNDER_ONLY, "description": "Master gate for the Integrations settings page and the connect/disconnect OAuth foundation for external accounts. Import-only by design; this foundation imports nothing yet."},
